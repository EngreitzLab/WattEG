...
//
// The per-simulation output is the only thing power can be re-derived from, so it is kept -- but as
...
// simulations x 10 columns, measured at ~2 GB peak for compute_power.R doing the same read. 16 GB
